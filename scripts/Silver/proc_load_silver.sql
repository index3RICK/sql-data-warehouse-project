...
BEGIN
    ...
    SELECT
        ...
        CASE 
            ...
            ELSE DATE_TRUNC('day', TO_DATE(sls_order_dt::TEXT,'YYYYMMDD'))::DATE
        END,
        CASE 
            WHEN sls_ship_dt = 0 OR LENGTH(sls_ship_dt::TEXT) <> 8 THEN NULL
            ELSE DATE_TRUNC('day', TO_DATE(sls_ship_dt::TEXT,'YYYYMMDD'))::DATE
        END,
        CASE
            WHEN sls_due_dt = 0 OR LENGTH(sls_due_dt::TEXT) <> 8 THEN NULL
            ELSE DATE_TRUNC('day', TO_DATE(sls_due_dt::TEXT,'YYYYMMDD'))::DATE
        END,
        CASE
        ...
        sls_quantity,
        CASE
            WHEN sls_price IS NULL OR sls_price <= 0 THEN COALESCE( sls_sales / NULLIF(sls_quantity,0), 0)
            ELSE sls_price
        END
    FROM bronze.crm_sales_details
    WHERE sls_order_dt > 19000101 AND LENGTH(sls_order_dt::text)=8;
